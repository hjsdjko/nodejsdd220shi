...
export default ({ config, db }) => {
	...
	api.all('/login', async (req, res) => {
		...
		try {
			...
			pwd = util.encryptmd5(req.query.password)
			let userinfo = await YonghuModel.findOne({ where: { yuangonggonghao: req.query.username, mima: pwd } })
			if (userinfo === null) {
			...
			const token = jwt.sign(
				{
					...
					yuangonggonghao:userinfo.dataValues.yuangonggonghao,
					mima:userinfo.dataValues.mima,
					yuangongxingming:userinfo.dataValues.yuangongxingming,
					xingbie:userinfo.dataValues.xingbie,
					touxiang:userinfo.dataValues.touxiang,
					shouji:userinfo.dataValues.shouji,
					zhuzhi:userinfo.dataValues.zhuzhi,
                    tableName: 'yonghu'
	...
	api.post('/register', async (req, res) => {
		...
		try {
			...
			req.body.mima = util.encryptmd5(req.body.mima)
	...
	api.all('/resetPass', async (req, res) => {
		...
		try {
			...
			let pwd = util.encryptmd5('123456')
	...
	api.post('/save',(req,res,next)=>{
		...
		try {
			...
			let yuangonggonghaocount = await YonghuModel.count({
				where: {
					yuangonggonghao: req.body["yuangonggonghao"]
				}
			})
            if (yuangonggonghaocount > 0) {
				toRes.session(res, -1, '员工工号已存在')
            }
		...
		try {
			let yuangonggonghaocount = await YonghuModel.count({
				where :{
					...
					yuangonggonghao:{
						[Op.eq]:req.body.yuangonggonghao
					}
				}
			})
            if (yuangonggonghaocount > 0) {
