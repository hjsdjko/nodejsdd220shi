...
import CangkuxinxiModel from '../models/CangkuxinxiModel'
...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let wupinfenlei = req.query.wupinfenlei
			if (wupinfenlei) {

				if (wupinfenlei.indexOf('%') != -1) {
					where.wupinfenlei = {
						[Op.like]: wupinfenlei
					}
				} else {
			...
			let guigexinghao = req.query.guigexinghao
			if (guigexinghao) {

				if (guigexinghao.indexOf('%') != -1) {
					where.guigexinghao = {
						[Op.like]: guigexinghao
					}
				} else {
					where.guigexinghao = {
						[Op.eq]: guigexinghao
			...
			let zhuyishixiang = req.query.zhuyishixiang
			if (zhuyishixiang) {

				if (zhuyishixiang.indexOf('%') != -1) {
					where.zhuyishixiang = {
						[Op.like]: zhuyishixiang
					}
				} else {
					where.zhuyishixiang = {
						[Op.eq]: zhuyishixiang
			...
			let wupinshuliang = req.query.wupinshuliang
			if (wupinshuliang) {

				if (wupinshuliang.indexOf('%') != -1) {
					where.wupinshuliang = {
						[Op.like]: wupinshuliang
					}
				} else {
					where.wupinshuliang = {
						[Op.eq]: wupinshuliang
			...
			let shiyongfanwei = req.query.shiyongfanwei
			if (shiyongfanwei) {

				if (shiyongfanwei.indexOf('%') != -1) {
					where.shiyongfanwei = {
						[Op.like]: shiyongfanwei
					}
				} else {
					where.shiyongfanwei = {
						[Op.eq]: shiyongfanwei
			...
			let wupinshuoming = req.query.wupinshuoming
			if (wupinshuoming) {

				if (wupinshuoming.indexOf('%') != -1) {
					where.wupinshuoming = {
						[Op.like]: wupinshuoming
					}
				} else {
					where.wupinshuoming = {
						[Op.eq]: wupinshuoming
			...
			let result = await CangkuxinxiModel.findAndCountAll({
				order: [orders],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.get('/lists', async (req, res) => {
		...
		try {
			let result = await CangkuxinxiModel.findAll()
	...
	api.get('/query', async (req, res) => {
		...
		try {
			...
			let result = await CangkuxinxiModel.findOne({where:dictionary})
	...
	api.post('/save',(req,res,next)=>{
		...
		try {
			...
			let wupinmingchengcount = await CangkuxinxiModel.count({
				where: {
					wupinmingcheng: req.body["wupinmingcheng"]
				}
			})
            if (wupinmingchengcount > 0) {
				toRes.session(res, -1, '物品名称已存在')
            }



			const userinfo = await CangkuxinxiModel.create(req.body)
	...
	api.post('/update',(req,res,next)=>{
		...
		try {
			...
			await CangkuxinxiModel.update(req.body, {
				where: {
				  id: req.body.id || 0
				}
			})
	...
	api.post('/delete',(req,res,next)=>{
		...
		try {
			...
			await CangkuxinxiModel.destroy({
				where: {
				  id: {
					[Op.in]: req.body
				  }
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await CangkuxinxiModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM cangkuxinxi WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM cangkuxinxi WHERE " + where + " AND " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM cangkuxinxi WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM cangkuxinxi WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM cangkuxinxi WHERE " + where + " AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM cangkuxinxi WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			// let tableName = "cangkuxinxi"
			let where = " WHERE 1 = 1 "
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM cangkuxinxi " + where + " GROUP BY " + columnName 
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			let where = " WHERE 1 = 1 "
			if ("cangkuxinxi" == "orders") {
				where += " AND status IN ('已支付', '已发货', '已完成') ";
			}

			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM cangkuxinxi " + where + " GROUP BY " + xColumnName + " DESC"
	...
	api.get('/count', async (req, res) => {
		...
		try {
			...
			var wupinfenlei = req.query.wupinfenlei
			if (wupinfenlei) {
			...
			var guigexinghao = req.query.guigexinghao
			if (guigexinghao) {
			...
			const count = await CangkuxinxiModel.count({where});
