...
import NiusheshiwuModel from '../models/NiusheshiwuModel'
...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let niushemianji = req.query.niushemianji
			if (niushemianji) {

				if (niushemianji.indexOf('%') != -1) {
					where.niushemianji = {
						[Op.like]: niushemianji
					}
				} else {
					where.niushemianji = {
						[Op.eq]: niushemianji
			...
			let shiwuleixing = req.query.shiwuleixing
			if (shiwuleixing) {

				if (shiwuleixing.indexOf('%') != -1) {
					where.shiwuleixing = {
						[Op.like]: shiwuleixing
					}
				} else {
					where.shiwuleixing = {
						[Op.eq]: shiwuleixing
			...
			let niusheweizhi = req.query.niusheweizhi
			if (niusheweizhi) {

				if (niusheweizhi.indexOf('%') != -1) {
					where.niusheweizhi = {
						[Op.like]: niusheweizhi
					}
				} else {
					where.niusheweizhi = {
						[Op.eq]: niusheweizhi
			...
			let shiwuriqi = req.query.shiwuriqi
			if (shiwuriqi) {

				if (shiwuriqi.indexOf('%') != -1) {
					where.shiwuriqi = {
						[Op.like]: shiwuriqi
					}
				} else {
			...
			let shiwuneirong = req.query.shiwuneirong
			if (shiwuneirong) {

				if (shiwuneirong.indexOf('%') != -1) {
					where.shiwuneirong = {
						[Op.like]: shiwuneirong
					}
				} else {
					where.shiwuneirong = {
						[Op.eq]: shiwuneirong
			...
			let result = await NiusheshiwuModel.findAndCountAll({
				order: [orders],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.get('/lists', async (req, res) => {
		...
		try {
			let result = await NiusheshiwuModel.findAll()
	...
	api.get('/query', async (req, res) => {
		...
		try {
			...
			let result = await NiusheshiwuModel.findOne({where:dictionary})
	...
	api.post('/save',(req,res,next)=>{
		...
		try {
			...
			const userinfo = await NiusheshiwuModel.create(req.body)
	...
	api.post('/update',(req,res,next)=>{
		...
		try {
			...
			await NiusheshiwuModel.update(req.body, {
				where: {
				  id: req.body.id || 0
				}
			})
	...
	api.post('/delete',(req,res,next)=>{
		...
		try {
			...
			await NiusheshiwuModel.destroy({
				where: {
				  id: {
					[Op.in]: req.body
				  }
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await NiusheshiwuModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM niusheshiwu WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM niusheshiwu WHERE " + where + " AND " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM niusheshiwu WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM niusheshiwu WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM niusheshiwu WHERE " + where + " AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM niusheshiwu WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			// let tableName = "niusheshiwu"
			let where = " WHERE 1 = 1 "
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM niusheshiwu " + where + " GROUP BY " + columnName 
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			let where = " WHERE 1 = 1 "
			if ("niusheshiwu" == "orders") {
				where += " AND status IN ('已支付', '已发货', '已完成') ";
			}

			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM niusheshiwu " + where + " GROUP BY " + xColumnName + " DESC"
