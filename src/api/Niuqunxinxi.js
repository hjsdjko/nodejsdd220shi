...
import NiuqunxinxiModel from '../models/NiuqunxinxiModel'
...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let niuqunshuliang = req.query.niuqunshuliang
			if (niuqunshuliang) {

				if (niuqunshuliang.indexOf('%') != -1) {
					where.niuqunshuliang = {
						[Op.like]: niuqunshuliang
					}
				} else {
					where.niuqunshuliang = {
						[Op.eq]: niuqunshuliang
					}
				}
			}
			...
			let niushebianhao = req.query.niushebianhao
			if (niushebianhao) {

				if (niushebianhao.indexOf('%') != -1) {
					where.niushebianhao = {
						[Op.like]: niushebianhao
					}
				} else {
					where.niushebianhao = {
						[Op.eq]: niushebianhao
			...
			let niushemingcheng = req.query.niushemingcheng
			if (niushemingcheng) {

				if (niushemingcheng.indexOf('%') != -1) {
					where.niushemingcheng = {
						[Op.like]: niushemingcheng
					}
				} else {
					where.niushemingcheng = {
						[Op.eq]: niushemingcheng
					}
				}
			}
			let siyangshuoming = req.query.siyangshuoming
			if (siyangshuoming) {

				if (siyangshuoming.indexOf('%') != -1) {
					where.siyangshuoming = {
						[Op.like]: siyangshuoming
					}
				} else {
					where.siyangshuoming = {
						[Op.eq]: siyangshuoming
					}
				}
			}
			...
			let result = await NiuqunxinxiModel.findAndCountAll({
				order: [orders],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.get('/lists', async (req, res) => {
		...
		try {
			let result = await NiuqunxinxiModel.findAll()
	...
	api.get('/query', async (req, res) => {
		...
		try {
			...
			let result = await NiuqunxinxiModel.findOne({where:dictionary})
	...
	api.post('/save',(req,res,next)=>{
		...
		try {
			...
			let niuqunbianhaocount = await NiuqunxinxiModel.count({
				where: {
					niuqunbianhao: req.body["niuqunbianhao"]
				}
			})
            if (niuqunbianhaocount > 0) {
				toRes.session(res, -1, '牛群编号已存在')
            }



			const userinfo = await NiuqunxinxiModel.create(req.body)
	...
	api.post('/update',(req,res,next)=>{
		...
		try {
			...
			await NiuqunxinxiModel.update(req.body, {
				where: {
				  id: req.body.id || 0
				}
			})
	...
	api.post('/delete',(req,res,next)=>{
		...
		try {
			...
			await NiuqunxinxiModel.destroy({
				where: {
				  id: {
					[Op.in]: req.body
				  }
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await NiuqunxinxiModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM niuqunxinxi WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM niuqunxinxi WHERE " + where + " AND " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM niuqunxinxi WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM niuqunxinxi WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM niuqunxinxi WHERE " + where + " AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM niuqunxinxi WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			// let tableName = "niuqunxinxi"
			let where = " WHERE 1 = 1 "
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM niuqunxinxi " + where + " GROUP BY " + columnName 
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			let where = " WHERE 1 = 1 "
			if ("niuqunxinxi" == "orders") {
				where += " AND status IN ('已支付', '已发货', '已完成') ";
			}

			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM niuqunxinxi " + where + " GROUP BY " + xColumnName + " DESC"
